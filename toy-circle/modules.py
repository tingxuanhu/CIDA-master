...
class FeatureNet(nn.Module):
    def __init__(self, opt):
        ...
        nx, nh = opt.nx, opt.nh   # nx: dimension of the input data     nh: dimension of hidden
...
class DiscNet(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.model in ['ADDA', 'CUA']:
            print('===> Discriminator Output Activation: sigmoid')
            self.output = lambda x: torch.sigmoid(x)
        else:
            print('===> Discriminator Output Activation: identity')
...
class PredNet(nn.Module):
    def __init__(self, opt):
        ...
        nh, nc = opt.nh, opt.nc  # num of hidden layers -- 800      num of labels -- 2
        nin = nh
        self.fc3 = nn.Linear(nin, nh)
        self.bn3 = nn.BatchNorm1d(nh)
        self.fc4 = nn.Linear(nh, nh)
